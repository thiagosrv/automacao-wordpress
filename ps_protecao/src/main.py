...
def main():
    print("=" * 60)
    print(" INICIANDO AUTOMACAO DE POSTAGENS: GOOGLE SHEETS -> WORDPRESS ")
    print("=" * 60)
    ...
    for index, row in enumerate(posts):
        # Define a chave unica para identificar se o post ja foi feito
        # Usamos o 'ID' da planilha. Se nao houver, usamos o 'Titulo'.
        row_id = row.get("ID") or row.get("Titulo")
        ...
        excerpt = (row.get("Excerpt") or row.get("Resumo") or row.get("Resumo do Post") or "").strip()
        tags = row.get("Tags", "").strip()
        categorias = row.get("Categorias", "").strip() or row.get("Categoria", "").strip()
        if not categorias or categorias.lower() == "geral":
            categorias = "Segurança Privada e Facilities"
        ...
        try:
            ...
            # Encerra a execucao apos publicar o proximo post da lista
            print("[+] Proximo post da lista publicado com sucesso. Encerrando execucao.")
            break
